# A guide for card enrollment with the iOS SDK <a style={{'border-bottom': '2px solid #0048ff'}} className="improve-docs" href="/select/sdks/ios/guide-v1">v1</a> <a style={{'margin-right': 'auto', 'color': '#111'}} className="improve-docs" href="/select/sdks/ios/guide-v2">v2</a>
...
- Download the latest version of the `Fidel.xcframework` artifact from the `master` branch of our [GitHub repo](https://github.com/Enigmatic-Smile/fidel-ios).

- Open your iOS app project in Xcode.

- Right click on your project and then on the `Add Files to "YourProjectName"...` button.

- Select the `Fidel.xcframework` artifact. Make sure to select the `Copy items if needed` option.
