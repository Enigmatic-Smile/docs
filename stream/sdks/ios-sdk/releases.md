...
- Useful features for corporate card enrollment, but not only:
  ...
  - The [`Fidel.verifyCard` function](https://transaction-stream.fidel.uk/docs/mobile-sdk#verifying-a-card-in-ios) to attempt card verification for a previously enrolled card. This function can be used for either the cardholder or a third-party entity (that cannot enroll cards, but can verify cards).
